...
*上传轮播图
*/
router.post('/rotate_do', (req, res) => {
    console.log("上传轮播图:rotate_do=================")
    console.log(req.body)
    // creates a new incoming form
    const form = new formidable.IncomingForm()
    //  文件上传文件夹
    const uploadPath = app.get('rotateUploadpath')
    ...
    form.parse(req, (err, fields, files) => {
        ...
        for (let attr in files){
            ...
            fileUrls.push(`http://${req.headers.host}/uploads/rotate/${filename}`)
        }
        // 返回结果
        return res.json({
            err: 1,
...
*删除轮播图
*/
router.get('/rotate_del', (req, res) => {
    console.log("删除轮播图:rotate_del=================")
    console.log(JSON.stringify(req.query));
    const filepath = path.join(app.get('rotateUploadpath'), req.query.filename)
